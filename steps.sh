...
## tearup
docker-compose -f $backing_service run --rm \
...
## tearup data
docker-compose -f $backing_service run --rm \
    -v $PWD/seed/tearup-acceptance.js:/tmp/tearup.js mongodb \
    mongo mongodb://mongodb:27017/fizzbuzzdb /tmp/tearup.js
## Acceptance Test
newman run atdd/fizzbuzz.postman_collection.json -d atdd/test-data/data.json
# or need to use data from csv format
# newman run atdd/fizzbuzz.postman_collection.json -d atdd/test-data/data.csv

## teardown data
docker-compose -f $backing_service run -v --rm \
    $PWD/seed/teardown-acceptance.js:/tmp/teardown.js mongodb \
    mongo mongodb://mongodb:27017/fizzbuzzdb /tmp/teardown.js
